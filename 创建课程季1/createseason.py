...
class CreateSeason():
    def __init__(self):
        ...
        self.driver, self.cookie = driver.get_login_driver("lsqiong_1", "lsq4590578",
                                                           self.url)
    ...
    def change_org(self, branch_name):
        try:
            branch_id = \
                dao(f'erp_prod', f"select id from tab_organization_info where org_name='{branch_name}'")[0]["ID"]
    ...
    def add_season(self, filename):
        ...
        for i in item:
            ...
            self.product_line = \
                dao("erp_prod", f"select product_line from tab_organization_info where org_name ='{self.branch_name}'")[
                    0][
                    "PRODUCT_LINE"]
...
if __name__ == '__main__':
    ...
    # print(pre_season)
    current_path = os.path.abspath(os.path.dirname(__file__))
    season.add_season(current_path + "/prod课程季.xlsx")
